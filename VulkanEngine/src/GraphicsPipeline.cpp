...
VulkanEngine::GraphicsPipeline::~GraphicsPipeline() {
  if( vk_graphics_pipeline ) {
    VulkanManager::getInstance()->getVkDevice().destroyPipeline( vk_graphics_pipeline );
  }
}
...
void VulkanEngine::GraphicsPipeline::bindPipeline() {
  ...
  current_command_buffer.bindPipeline( vk::PipelineBindPoint::eGraphics, vk_graphics_pipeline );
}
...
void VulkanEngine::GraphicsPipeline::setScissor(
  ...
  vk_scissor = vk::Rect2D()
    ...
    .setExtent( vk::Extent2D( width_x, width_y ) );
...
void VulkanEngine::GraphicsPipeline::createGraphicsPipeline(
  ...
  vk_graphics_pipeline = VulkanManager::getInstance()->getVkDevice().createGraphicsPipeline( nullptr, graphics_pipeline_info ).value;
}
