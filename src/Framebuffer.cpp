...
VulkanEngine::Framebuffer::Framebuffer(
  ...
  vk_framebuffer = VulkanManager::getInstance().getDevice()->getVkDevice().createFramebuffer(
      framebuffer_info);
}

VulkanEngine::Framebuffer::~Framebuffer() {
  VulkanManager::getInstance().getDevice()->getVkDevice().destroyFramebuffer(vk_framebuffer);
}
