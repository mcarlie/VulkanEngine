...
namespace VulkanTest {
  ...
  /// staging functionality for that class. E.g the type StagedBuffer< Image > is an Image which has staging functionality
  /// \tparam DestinationClass The class which will be the receiver of the data in this buffer
  /// \tparam DestinationClassArgs Parameter pack specifying the signature of the constructor of DestinationClass
  ...
  public:
    ...
    /// \param command_buffer The command buffer to record the transfer command in.
    ///                       If not specified an internal command buffer will be created
    ///                       and submitted to the appropriate queue
  ...
  protected:
    ...
    /// The source buffer. Data will be transferred from this buffer to the destination
    /// buffer when calling transferBuffer()
